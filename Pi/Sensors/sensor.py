...
class Measurement(object):
    ...
    def raw_distance(self, sample_size=11, sample_wait=0.1):
        ...
        for distance_reading in range(sample_size):
            ...
            timeout = sonar_signal_off + self.maxTime
            while GPIO.input(self.echo_pin) == 0 and sonar_signal_off < timeout:
                sonar_signal_off = time.time()
                echo_status_counter += 1
            sonar_signal_on = time.time()
            timeout = sonar_signal_on + self.maxTime
            while GPIO.input(self.echo_pin) == 1 and sonar_signal_on < timeout:
                sonar_signal_on = time.time()
